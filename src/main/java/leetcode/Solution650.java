 ...
 * on 2018/09/03.
 */
public class Solution650 extends BaseTest {
    ...
    public int minSteps(int n) {
        if (n == 1) return 0;
        int[][][] cache = new int[n + 1][n + 1][n + 1];
        int i = 1 + minSteps(n - 1, 1, 1, cache);
        return i;
    }
    ...
    private int minSteps(int remainN, int paperN, int copyANumber, int[][][] cache) {
        ...
        if (cache[copyANumber][paperN][remainN] != 0) return cache[copyANumber][paperN][remainN];
        ...
        return cache[copyANumber][paperN][remainN] = minStep;
    }
}
